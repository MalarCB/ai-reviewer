...
export const OVERVIEW_MESSAGE_SIGNATURE =
  "\n<!-- dsadcodereview.ai: overview message -->";

export const COMMENT_SIGNATURE = "\n<!-- dsadcodereview.ai: comment -->";

export const PAYLOAD_TAG_OPEN = "\n<!-- dsadcodereview.ai: payload --";
export const PAYLOAD_TAG_CLOSE = "\n-- dsadcodereview.ai: payload -->";

const DSADCODEREVIEW_SIGNATURE = "--- \n_autogenerated by dsadcodereview.ai (https://github.com/cbdr/dsad-ai-reviewer)_";
...
export function buildLoadingMessage(
  ...
  for (const commit of commits.reverse()) {
    message += `- [${commit.sha.slice(
      0,
      7
    )}](https://github.com/${owner}/${repo}/commit/${commit.sha}): ${commit.commit.message
      }\n`;
  }
  ...
  for (const diff of fileDiffs) {
    ...
    fileText += ` _(${diff.hunks.length} ${diff.hunks.length === 1 ? "hunk" : "hunks"
      })_`;
    message += `${fileText}\n`;
  }
  message += "\n</details>\n\n";

  message += DSADCODEREVIEW_SIGNATURE;
